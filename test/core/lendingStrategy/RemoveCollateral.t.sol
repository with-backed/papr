...
contract RemoveCollateralTest is BaseLendingStrategyTest {
    ...
    event RemoveCollateral(uint256 indexed vaultId, ILendingStrategy.Collateral collateral, uint256 vaultDebt);
    ...
    function testUpdatesPricesCorrectly() public {
        _addCollateral();
        strategy.removeCollateral(address(1), vaultNonce, collateral);
        assertEq(0, strategy.collateralFrozenOraclePrice(strategy.collateralHash(collateral, vaultId)));
